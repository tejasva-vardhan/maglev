...
WITH RECURSIVE
    ...
    horizon(cap) AS (
        ...
        FROM (SELECT MAX(sqlc.arg(ref_date), strftime('%Y%m%d', 'now')) AS anchor)
    ),
    ...
    candidate_date(d) AS (
        SELECT strftime('%Y%m%d', date(substr(sqlc.arg(ref_date), 1, 4) || '-' || substr(sqlc.arg(ref_date), 5, 2) || '-' || substr(sqlc.arg(ref_date), 7, 2), '+1 day'))
