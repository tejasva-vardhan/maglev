...
func ParseTimeParameter(timeParam string, currentLocation *time.Location) (string, time.Time, map[string][]string, bool) {
	...
	if epochTime, err := strconv.ParseInt(timeParam, 10, 64); err == nil {
		...
		// then fall back to yyyy-MM-dd (e.g. "2024-03-15").
		if parsed, dtErr := time.ParseInLocation("2006-01-02_15-04-05", timeParam, currentLocation); dtErr == nil {
			parsedTime = parsed
			validFormat = true
		} else if parsed, dErr := time.ParseInLocation("2006-01-02", timeParam, currentLocation); dErr == nil {
			parsedTime = parsed
			validFormat = true
		}
	}
