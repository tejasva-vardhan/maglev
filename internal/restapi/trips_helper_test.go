...
// Regression tests for calculateScheduleDeviationFromTripUpdates.
// The bug was using int64(*delay) (nanoseconds) instead of int64(delay.Seconds()),
// causing a 90-second delay to appear as 90,000,000,000.

func TestCalculateScheduleDeviationFromTripUpdates_NoUpdates(t *testing.T) {
	api := createTestApi(t)
	defer api.Shutdown()

	deviation := api.calculateScheduleDeviationFromTripUpdates("nonexistent-trip")
	assert.Equal(t, 0, deviation)
}

func TestCalculateScheduleDeviationFromTripUpdates_ArrivalDelayInSeconds(t *testing.T) {
	api := createTestApi(t)
	defer api.Shutdown()
	t.Cleanup(api.GtfsManager.MockResetRealTimeData)
	...
	api.GtfsManager.MockAddTripUpdate("trip-arrival-seconds", nil, updates)

	deviation := api.calculateScheduleDeviationFromTripUpdates("trip-arrival-seconds")
	// Must be 90 (seconds), not 90_000_000_000 (nanoseconds).
	assert.Equal(t, 90, deviation, "deviation must be in seconds, not nanoseconds")
}

func TestCalculateScheduleDeviationFromTripUpdates_DepartureDelayFallback(t *testing.T) {
	api := createTestApi(t)
	defer api.Shutdown()
	t.Cleanup(api.GtfsManager.MockResetRealTimeData)
	...
	api.GtfsManager.MockAddTripUpdate("trip-departure-seconds", nil, updates)

	deviation := api.calculateScheduleDeviationFromTripUpdates("trip-departure-seconds")
	assert.Equal(t, 120, deviation, "departure delay should be used when no arrival delay is present")
}

func TestCalculateScheduleDeviationFromTripUpdates_ArrivalTakesPrecedenceOverDeparture(t *testing.T) {
	api := createTestApi(t)
	defer api.Shutdown()
	t.Cleanup(api.GtfsManager.MockResetRealTimeData)
	...
	departureDelay := 180 * time.Second
	updates := []gtfs.StopTimeUpdate{
		{
			StopID:    &stopID,
	...
	api.GtfsManager.MockAddTripUpdate("trip-arrival-precedence", nil, updates)

	deviation := api.calculateScheduleDeviationFromTripUpdates("trip-arrival-precedence")
	assert.Equal(t, 60, deviation, "arrival delay should take precedence over departure delay")
}

func TestCalculateScheduleDeviationFromTripUpdates_NoDelayOnStop(t *testing.T) {
	api := createTestApi(t)
	defer api.Shutdown()
	t.Cleanup(api.GtfsManager.MockResetRealTimeData)
	...
	updates := []gtfs.StopTimeUpdate{
		{
			StopID:  &stopID,
			Arrival: &gtfs.StopTimeEvent{}, // Delay is nil
		},
	}
	api.GtfsManager.MockAddTripUpdate("trip-no-delay", nil, updates)

	deviation := api.calculateScheduleDeviationFromTripUpdates("trip-no-delay")
	assert.Equal(t, 0, deviation)
}
